...
/* # This outputs the input name and value to the local scripts directory
 ...
 { # Begin building name tests
   ...
   if(preg_match("/^[0-9a-z\-]{1,14}[0-9a-z]$/i", $input_value))
	 { # If the name is valid, check to see if it exists
		...
		$query = "SELECT hosts.name,hosts.subnet,subnet.subnet,subnet.zone FROM hosts,subnet WHERE name=? and hosts.subnet=subnet.subnet";
		...
		if(isset($result))
		 { # Begin dealing with results
			if (count($result) > '1')
			 ...
			 { # Begin single result
				$zonelist = $result[0]['zone'];
			 } # End single result
				echo '<span style="color:red;">Host "' . $result[0]['name'] . '" exists in zone ' . $zonelist . '</span>';
...
if($input_name == 'ip')
 { # Begin building IP tests
	...
	# Since a host name is necessary for a CNAME, see if it's one of those.
	elseif(preg_match("/^([a-z0-9][a-z0-9-]*?[a-z0-9]\.)+[a-z0-9]{2,}$/", $input_value))
	 { # Begin checking CNAME
		$dbh = new db_iface('database');
      $query = "SELECT name,ip FROM hosts WHERE ip=?";
      $query_term[] = $input_value;
      ...
      if(isset($result))
       { # Begin dealing with results
			echo '<span style="color:red">This host already has a CNAME.</span>';
		 } # End dealing with results
		else { echo '<span style="color:red">Valid CNAME, invalid IP</span>'; }
	 } # End checking CNAME
