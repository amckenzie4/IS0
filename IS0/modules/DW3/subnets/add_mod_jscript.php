...
// User created functions go here.
	...
	// Declare the findMask function
	function findMask(subnet,divId)
			...
			request.onreadystatechange=function()
			 {
				if (request.readyState==4 && request.status==200)
				 {
					//dataToDiv(divId,request.responseText);
					document.getElementById('add_mod_form').elements.namedItem('mask').value = request.responseText;
				 }
			 }

			// Set the query URL
			var url = "modules/subnets/scripts/find_mask.php?q="+subnet;
//document.getElementById("debug").innerHTML = "url is " + url + "<br> and divId is " + divId;
